...
export default function EditExerciceOrthographe() {
    ...
    async function loadExercice() {
        try {
            ...
            if (!token) { router.push('/aclef-pedagogie-admin'); return }

            const res = await fetch(`/api/admin/ecrire/orthographe/exercices/${id}`, {
            ...
            setForm({
                ...
                niveau: ex.niveau || 'A',
                numero_boite: ex.numero_boite || '',
                difficulte: ex.difficulte || 'moyen',
                competence_id: ex.competence_id || '',
                actif: ex.actif !== false
            })
            setContenu(ex.contenu || {})
        } catch (err) {
    ...
    async function handleSave() {
        if (!form.titre || !form.theme_grammatical || !form.type) {
    ...
    // ============================================================
    // ÉDITEURS DE CONTENU (identiques à nouveau.js)
    // ============================================================
                ...
                <div style={styles.optionsRow}>
                    {(c.options || []).map((opt, i) => (
                        <div key={i} style={styles.optionItem}>
                            ...
                            {c.options.length > 1 && (
                                <button onClick={() => {
                                    setContenu({ ...c, options: c.options.filter((_, j) => j !== i) })
    ...
    function renderContenuEditor() {
        switch (form.type) {
            ...
            default: return <p>Type d'exercice non reconnu</p>
        }
    }

    if (loading) return <div style={{ padding: '40px', textAlign: 'center', color: '#666' }}>Chargement...</div>

    const typeInfo = TYPES_EXERCICES.find(t => t.value === form.type)

    return (
        <div style={styles.container}>
            <div style={styles.header}>
                <h1 style={styles.title}>Modifier l'exercice</h1>
            ...
            <div style={styles.formCard}>
                <div style={styles.infoBanner}>
                    ...
                    <span style={{ color: '#64748b' }}> — Le type ne peut pas être modifié</span>
                </div>
                ...
                <div style={styles.formGroup}>
                    ...
                    <input value={form.titre} onChange={(e) => setForm({ ...form, titre: e.target.value })} style={styles.input} />
                </div>

                <div style={styles.formGroup}>
                    <label style={styles.label}>Consigne</label>
                    <input value={form.consigne} onChange={(e) => setForm({ ...form, consigne: e.target.value })} style={styles.input} />
                </div>
                ...
                <div style={styles.formRow}>
                    ...
                    <div style={styles.formGroup}>
                        <label style={styles.label}>N° boîte</label>
                        <input type="number" value={form.numero_boite} onChange={(e) => setForm({ ...form, numero_boite: e.target.value })} style={styles.input} />
                    </div>
                    <div style={styles.formGroup}>
                        <label style={styles.label}>Actif</label>
                        <select value={form.actif ? 'true' : 'false'} onChange={(e) => setForm({ ...form, actif: e.target.value === 'true' })} style={styles.input}>
