...
export default async function handler(req, res) {
    ...
    try {
        ...
        const { element_id, mot, commentaire, question, reponse, image_url } = req.body
        ...
        if (imagier.created_by !== user.id) {
            return res.status(403).json({ message: 'Non autorisé à modifier cet élément' })
        }
        ...
        if (updateError) {
            ...
            return res.status(500).json({ message: 'Erreur lors de la mise à jour de l\'élément' })
        }
