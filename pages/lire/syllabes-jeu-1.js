...
export default function JeJoueSyllabes() {
    ...
    const prepareGame = (allWords) => {
        ...
        // Sélectionner 10 mots au hasard avec des premières syllabes différentes
        const selectedWords = []
        const usedFirstSyllables = new Set()
        ...
        for (const word of shuffledWords) {
            ...
            const firstSyllable = syllables[0].toLowerCase()
            const wordLower = word.mot.toLowerCase()

            // Vérifier que ce mot n'est pas déjà utilisé et que cette première syllabe n'est pas déjà utilisée
            if (!usedWords.has(wordLower) && !usedFirstSyllables.has(firstSyllable)) {
                usedWords.add(wordLower)
                usedFirstSyllables.add(firstSyllable)
                selectedWords.push({
                    original: word.mot,
                    syllables: syllables,
                    firstSyllable: syllables[0],
                    remainingSyllables: syllables.slice(1).join(''),
                    panier: word.panier,
                    lettre: word.lettre,
                    isCompleted: false,
        ...
        if (selectedWords.length < 10) {
            alert('Pas assez de mots uniques avec des premières syllabes différentes.\nOrganisez plus de mots différents dans vos paniers !')
            return
        }
        ...
        const syllablesArray = shuffledSelectedWords.map(word => word.firstSyllable)
            ...
            <div style={{
                ...
                {window.innerWidth > 768 && (
                    <h1 style={{
                        ...
                        🎮 Jeu 4 - Je joue avec les syllabes - Début
                    </h1>
                )}
                ...
                {!gameStarted ? (
                    ...
                    <div style={{
                        ...
                        <p style={{
                            ...
                            Dans ce jeu, vous devez replacer la première syllabe de 10 mots issus de vos textes personnels.
                            <br />
                                ...
                                {shuffledSyllables
                                    .filter(syllable => {
                                        ...
                                        return gameWords.some(word =>
                                            word.firstSyllable === syllable && !word.isCompleted
                                        )
                                    })
                                    .map((syllable, index) => (
                                        <div
                                    ...
                                    <div style={{
                                        ...
                                        {word.isCompleted ? (
                                            ...
                                            <>
                                                ...
                                                <span>{word.remainingSyllables}</span>
                                            </>
                                        )}
                                    </div>
