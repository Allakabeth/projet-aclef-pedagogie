...
// Convertir un nombre en mots avec tirets (pour exercice mono/multi syllabes)
export function convertNumberToWordsWithHyphens(num) {
    ...
    // Si le nombre est trop grand, retourner null
    if (parsedNum > 999999) {
        return null
    }

    // Convertir en mots
    const words = convertNumberToWords(parsedNum)

    // Remplacer tous les espaces par des tirets
    const withHyphens = words.replace(/\s+/g, '-')
