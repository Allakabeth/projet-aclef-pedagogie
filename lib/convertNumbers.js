...
export function convertNumbersInText(text) {
    ...
    // Regex pour trouver les nombres (entiers et décimaux)
    return text.replace(/\b\d+(?:[,.]\d+)?\b/g, (match) => {
        // Gérer les décimaux avec virgule ou point
        ...
        if (match.includes(',') || match.includes('.')) {
            ...
            if (decimalPart) {
                result += ' virgule '
                // Lire chaque chiffre après la virgule
                for (let digit of decimalPart) {
                    result += unites[parseInt(digit)] + ' '
                }
                result = result.trim()
            }
